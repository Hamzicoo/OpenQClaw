...
> 根据社区反馈，目前 QClaw 已临时阻断当前版本直接调用无限量默认模型的 API，经测试确认可通过**配置自定义模型**继续正常使用 OpenQClaw。
> 
> OpenQClaw 0.1.2 现已同步提供 **Apple Silicon (arm64)** 和 **Intel (x64)** 版本，并移除了启动时的强制更新检查机制。
> 
> 💡 遇到连接失败 / 强制更新弹窗阻拦 / API 报错 403 / 微信远程控制失效等问题，**请务必查阅 [👉 FAQ 常见问题解答](FAQ.md)**。
